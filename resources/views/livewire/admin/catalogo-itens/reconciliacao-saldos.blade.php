<div class="report-canvas">
    ...
    <x-report-card padding="p-0">
                    ...
                    @forelse ($saldos as $saldo)
                        <tr class="align-top transition-colors hover:bg-slate-800/40">
                                ...
                                @forelse ($sugestoes[$saldo->id] ?? [] as $sugestao)
                                    <div class="mb-1 flex items-center gap-2">
                                        <span class="inline-flex rounded px-2 py-0.5 text-xs font-medium
                                            {{ $sugestao['confianca'] === 'alta' ? 'bg-emerald-500/15 text-emerald-400' : ($sugestao['confianca'] === 'media' ? 'bg-amber-500/15 text-amber-400' : 'bg-slate-700/60 text-slate-400') }}">
                                            {{ ucfirst($sugestao['confianca']) }} ({{ number_format($sugestao['score'] * 100, 0) }}%)
                                        </span>
                                        <span class="text-slate-300">{{ $sugestao['item']->descricao }}</span>
                                        <button wire:click="vincular({{ $saldo->id }}, {{ $sugestao['item']->id }})" class="text-xs text-blue-400 hover:text-blue-300">Confirmar</button>
                                    </div>
                                @empty
                            ...
                            <td class="px-4 py-3 text-right">
                                <button wire:click="abrirVinculoManual({{ $saldo->id }})" class="text-xs text-blue-400 hover:text-blue-300">Buscar manualmente</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
        ...
        <div class="border-t border-slate-800 px-4 pb-4 pt-3">
            {{ $saldos->links() }}
        </div>
    </x-report-card>
    ...
    @if ($saldoSelecionadoId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div class="w-full max-w-lg rounded-xl border border-slate-800 bg-slate-900 p-6 text-slate-100 shadow-xl">
                ...
                <input
                    ...
                    class="mb-4 w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-600"
                />
                ...
                <div class="max-h-80 space-y-2 overflow-y-auto">
                    @forelse ($itensBuscaManual as $item)
                        <div class="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2">
                            ...
                            <button wire:click="vincular({{ $saldoSelecionadoId }}, {{ $item->id }})" class="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-500">Vincular</button>
                        </div>
                    @empty
                ...
                <div class="mt-4 flex justify-end">
                    <button wire:click="fecharVinculoManual" class="rounded-lg border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-medium text-slate-200 hover:bg-slate-700">
                        Fechar
                    </button>
                </div>
            </div>
