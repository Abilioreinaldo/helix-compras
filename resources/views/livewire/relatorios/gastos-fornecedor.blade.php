<div class="p-6">
    <h1 class="text-2xl font-bold mb-6">Gastos por Fornecedor</h1>

    <div class="flex gap-4 mb-6">
        <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">Agrupar por</label>
            <select wire:model.live="agrupamento" class="border-gray-300 rounded shadow-sm text-sm">
            ...
            <table class="min-w-full bg-white border rounded-lg shadow-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        @if($porCategoria)
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Nº Fornecedores</th>
                        @else
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach($resultados as $linha)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-sm font-medium">{{ $linha->rotulo }}</td>
                            @if($porCategoria)
                                <td class="px-4 py-3 text-sm text-right">{{ $linha->total_fornecedores }}</td>
                            @else
                                <td class="px-4 py-3 text-sm text-gray-500">{{ $linha->categoria }}</td>
                            @endif
