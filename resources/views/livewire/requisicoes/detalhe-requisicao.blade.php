<div class="report-canvas">
            ...
            <dl class="space-y-2 text-sm">
                ...
                @if ($requisicao->justificativa)
                    <div>
                        ...
                        <dd class="text-slate-200 bg-slate-950/40 border border-slate-800 p-2 rounded text-xs">{{ $requisicao->justificativa }}</dd>
                    </div>
                @endif
            </dl>
        </x-report-card>
    ...
    <x-report-card title="Histórico">
        <ol class="relative border-l border-slate-800 ml-3 space-y-4">
            @foreach ($requisicao->logs->sortBy('created_at') as $log)
                <li class="ml-4">
                    <div class="absolute w-2 h-2 bg-slate-600 rounded-full -left-1 mt-1.5"></div>
    ...
    @if ($mostrarModalCancelar)
        <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <div class="bg-slate-900 border border-slate-800 rounded-xl shadow-xl w-full max-w-md p-6">
