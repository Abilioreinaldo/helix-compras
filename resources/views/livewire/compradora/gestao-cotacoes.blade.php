<div class="report-canvas">
    ...
    >
        <x-slot:actions>
            <a href="{{ route('compradora.mapa-cotacao', ['requisicaoId' => $requisicao->id]) }}"
                class="rounded-lg border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-medium text-slate-200 hover:bg-slate-700">
                Mapa comparativo
            </a>
        </x-slot:actions>
    </x-page-header>
    ...
    <x-report-card title="Progresso">
        <div class="flex flex-wrap items-center gap-8">
            ...
            <div class="ml-auto">
                ...
                @else
                    <button disabled class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-slate-500 cursor-not-allowed">
                        Concluir Cotação
                    </button>
                @endif
            </div>
        ...
        <x-report-card title="Nova Cotação">
            ...
            <div class="flex justify-end gap-3 mt-4">
                <button wire:click="$set('mostrarFormulario', false)"
                    class="rounded-lg bg-slate-800 border border-slate-700 px-4 py-2 text-sm text-slate-200 hover:bg-slate-700">
                    Cancelar
                </button>
    ...
    @if ($mostrarModalConcluir)
        <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <div class="rounded-lg border border-slate-800 bg-slate-900 shadow-xl w-full max-w-md p-6">
