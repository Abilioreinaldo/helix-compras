...
<div x-data="{ open: false }" class="flex h-screen overflow-hidden">
    ...
    <aside
        ...
        class="fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-slate-800 bg-slate-900 transition-transform duration-200 md:static md:translate-x-0"
    >
        <div class="flex h-16 items-center gap-3 border-b border-slate-800 px-5">
            <x-helix-logo class="h-9 w-9" />
            <div class="leading-tight">
                <p class="text-sm font-bold tracking-[0.2em] text-white">HELIX</p>
        ...
        <div class="border-t border-slate-800 p-3">
            <div class="flex items-center gap-3 rounded-lg px-2 py-1.5">
                <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-800 text-sm font-semibold text-blue-400">{{ $iniciais ?: '·' }}</span>
                <div class="min-w-0">
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
        ...
        {{-- Topbar --}}
        <header class="flex h-16 flex-shrink-0 items-center gap-4 border-b border-slate-800 bg-slate-900 px-4 md:px-6">
