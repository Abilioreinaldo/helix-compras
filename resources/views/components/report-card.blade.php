...
<div {{ $attributes->merge(['class' => 'overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 shadow-sm']) }}>
    @if($title || isset($actions))
        <div class="flex items-center justify-between gap-4 border-b border-zinc-800 px-5 py-4">
            <div class="flex items-center gap-3">
                @if($icon)
                    <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-zinc-800 text-emerald-400">
