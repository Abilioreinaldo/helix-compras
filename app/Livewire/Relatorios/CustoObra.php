...
class CustoObra extends Component
{
    ...
    public function render(): View
    {
        ...
        // strftime('%m', ...) é SQLite nativo; produção usa SQLite conforme CLAUDE.md.
        $movimentos = DB::table('itens_pedido_compra as ipc')
            ...
            ->select(
                ...
                DB::raw("strftime('%m', pc.emitido_em) as mes"),
                DB::raw('SUM(ipc.valor_total) as total_mes'),
            )
            ->groupBy('o.id', 'o.verba', 'un.nome', DB::raw("strftime('%m', pc.emitido_em)"))
            ->orderBy('un.nome')
            ->orderBy('mes')
            ->get();
