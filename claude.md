...
\- Nunca leia ou modifique .env

\- Nunca rode migrate:fresh sem confirmar comigo

\- Não adicione dependência sem justificar

\- Banco: SQLite local em desenvolvimento
...
- **Isolamento por arquitetura:** toda model com `tenant_id` usa `BelongsToTenant`. Proibido `withoutGlobalScopes()`; bypass só com `Model::withoutTenantScope()` + filtro explícito (superadmin/relatório global). Jobs, comandos e webhooks estabelecem o tenant com `TenantContext::runFor($tenantId, ...)`. Modo estrito e carimbo protegido ligados desde o dia 1 e **literais** no `config/foundation.php` do app (`'strict' => true, 'enforce_stamp' => true`) — nunca por `env()`: variável vazia vira `false` e desliga o fail-closed em silêncio. `php artisan helix:doctor --strict-env=production` é step bloqueante do CI.
